<template>
	<view class="content">
		<form class="app-update-pv">
			<view
				...
				<input
					...
					v-model="ruleForm.qingjiaTime" placeholder="请假时间" :readonly="ro.qingjiaTime"
			...
			<view
				...
				<input disabled
					:style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"#A27DE3","borderRadius":"0px","color":"#A27DE3","textAlign":"left","borderWidth":"0 0 6rpx","fontSize":"24rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
					:disabled="ro.qingjiaNumber" type="number" v-model="ruleForm.qingjiaNumber"
					placeholder="请假时间(天)"></input>
			</view>
			<!-- text后缀 -->
			<view
				...
				<textarea
					...
					:disabled="ro.qingjiaText" v-model="ruleForm.qingjiaText" placeholder="请假原因" />
			</view>
		...
		<w-picker mode="date" step="1" :current="false" :hasSecond="false" @confirm="qingjiaTimeConfirm"
	...
	export default {
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if ((!this.ruleForm.qingjiaNumber) && (!this.$validate.isIntNumer(this.ruleForm.qingjiaNumber)) && this
					.ruleForm.qingjiaNumber > 0) {
					this.$utils.msg(`请假时间(天)不能为空，不能小于0 格式为数字`);
					return
				}
