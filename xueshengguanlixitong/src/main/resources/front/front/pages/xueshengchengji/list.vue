<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.xueshengchengjiName" type="text" placeholder="科目"></input>
		</view>
	...
	<view class="list b-b" v-for="(item, index) in list" :key="index">
		<view class="wrapper" style="margin-left: 25upx;margin-top: 10upx;">
			<view style="font-size: 30rpx;">
				<b>{{item.xueshengchengjiName}}</b>
			</view>
			<view class="address-box" style="margin-bottom: 10rpx;width: 600rpx;">
				<text style="margin-left: 30rpx;">发布教师：{{item.jiaoshiName}}</text>
			</view>
			<view class="address-box" style="margin-bottom: 10rpx;width: 600rpx;">
				<text style="margin-left: 30rpx;">学生成绩：{{item.xueshengchengji}}</text>
			</view>
			<view class="address-box" style="margin-bottom: -10rpx;width: 600rpx;">
				<text style="margin-left: 30rpx;">成绩详情：{{item.xueshengchengjiContent}}</text>
			</view>
			<!--<view class="u-box" style="color: #8799A3;">-->
			<!--</view>-->
		</view>
        <view style="float: right;">
            <text v-if="isAuth('xueshengchengji','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(item.id)"></text>
            <text v-if="isAuth('xueshengchengji','删除')" style="margin: 0 50upx 0 30rpx;" class="cuIcon-delete" @click.stop="onDeleteTap(item.id)"></text>
        </view>
	...
	<button
            ...
            v-if="isAuth('xueshengchengji','新增')" class="add-btn" @click="onAddTap()">新增</button>
...
<script>
	export default {
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.xueshengchengjiName = "";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
                ...
                let res = await this.$api.page(`xueshengchengji`, params);
			...
			async search() {
				...
				if (this.searchForm.xueshengchengjiName) {
                    searchForm['xueshengchengjiName'] = this.searchForm.xueshengchengjiName
                }
				if (this.searchForm.xueshengchengjiName) {
					searchForm['xueshengchengjiName'] = this.searchForm.xueshengchengjiName
				}

                let res = await this.$api.page(`xueshengchengji`, searchForm);
				// 如果是第一页数据置空
